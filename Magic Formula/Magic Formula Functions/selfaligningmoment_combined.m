function M_z = selfaligningmoment_combined(Xcell,alpha,F_z,p_i,gamma,Mz_Fycell,kappa,Mz_Fxcell)
%% Inputs

gamma = gamma.*0.0174533;  %degrees to radians
alpha_m = alpha*0.0174533;        

R_0 = 6.5;  %unloaded tire radius
...
[p_pz1,p_pz2,q_bz1,q_bz2,q_bz3,q_bz4,q_bz5,q_bz9,q_bz10,q_cz1,q_dz1,q_dz2,q_dz3,q_dz4,q_dz6,q_dz7,q_dz8,q_dz9,q_dz10,q_dz11,...
    q_ez1,q_ez2,q_ez3,q_ez4,q_ez5,q_hz1,q_hz2,q_hz3,q_hz4,s_sz1,s_sz2,s_sz3,s_sz4] = Xcell{:};

lambda_t = 1;        % pneumatic trail (affecting aligning torque stiffness)    
lambda_r = -5;        % curvature 
lambda_kyalpha = 1;  % cornering stiffness
lambda_kzgamma = 1;  % camber force stiffness
lambda_muy = 1;      % peak friction coefficient

lambda_s = 1; % moment arm of F_x

[S_hy,S_vy,K_yalpha,B_y,C_y,F_y,G_ykappa0] = Mz_Fycell{:}; %lateral force parameters
[K_xkappa,F_x] = Mz_Fxcell{:};
...
% Combined slip
alpha_teq = atan(sqrt(tan(alpha_t).^2+(K_xkappa./K_yalpha).^2.*kappa.^2).*sign(alpha_t));
alpha_req = atan(sqrt(tan(alpha_r).^2+(K_xkappa./K_yalpha).^2.*kappa.^2).*sign(alpha_r));
s = (s_sz1+s_sz2.*(F_y./F_z0)+(s_sz3+s_sz4.*df_z).*gamma).*R_0.*lambda_s;
...
M_z = transpose(-t.*F_y.*G_ykappa0+M_zr+s.*F_x);
