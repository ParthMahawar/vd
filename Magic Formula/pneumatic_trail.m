function t = pneumatic_trail(Xcell,alpha,F_z,p_i,gamma,Mz_Fycell)
%% Inputs
...
t = transpose(D_t.*cos(C_t.*atan(B_t.*alpha_teq-E_t.*(B_t.*alpha_teq-atan(B_t.*alpha_teq)))).*cos(alpha_m));
