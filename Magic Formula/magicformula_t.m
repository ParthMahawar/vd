...
for j = 1:NP
    Xeval = num2cell(X(j,:));
    MzXeval(j,:) = pneumatic_trail(Xeval,alpha,Fz,pi,gamma,Mz_Fycell);
end
errorXeval = sum((MzXeval - repmat(transpose(Mz./Fy),NP,1)).^2,2); 
...
Xicell = num2cell(Xi);
MzXi = pneumatic_trail(Xicell,alpha,Fz,pi,gamma,Mz_Fycell);

Xnicell = num2cell(Xni);
MzXni = pneumatic_trail(Xnicell,alpha,Fz,pi,gamma,Mz_Fycell);

errorXi = sum((MzXi - transpose(Mz./Fy)).^2); 
errorXni = sum((MzXni - transpose(Mz./Fy)).^2); 
...
if (iterations == 1 || mod(iterations,5) == 0) && liveplotting == 1
    ...
    scatter(alpha(1:end),Mz./Fy(1:end));
    hold on

    Mzplot = pneumatic_trail(Xbestcell,alpha,Fz,pi,gamma,Mz_Fycell);
...
Mzplot2 = pneumatic_trail(Xbestcell,alpha2,Fz2,pi2,gamma2,Mz_Fycell2);
...
if plot3 == 1
    ...
    for a = 1:numel(P_input3)
        for b = 1:numel(IA_input3)
            for c = 1:numel(FZ_input3)
                Mzplot3 = pneumatic_trail(Xbestcell,alpha3,Fz3(:,c),pi3(:,a),gamma3(:,b),Mz_Fycell3);
