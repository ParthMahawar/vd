...
normal_load_RL = roll_angle*((C.rear_spring_stiff)*(C.rear_width/2)+((C.rear_ARB_stiff)/(C.rear_width/2)))+(C.weight_dist)*(C.mass/2);
normal_load_RR = -roll_angle*((C.rear_spring_stiff)*(C.rear_width/2)+((C.rear_ARB_stiff)/(C.rear_width/2)))+(C.weight_dist)*(C.mass/2);

%Normal Load on front wheels as a function of roll angle
normal_load_FL = roll_angle*((C.front_spring_stiff)*(C.front_width/2)+((C.front_ARB_stiff)/(C.front_width/2)))+(C.weight_dist)*(C.mass/2);
normal_load_FR = -roll_angle*((C.front_spring_stiff)*(C.front_width/2)+((C.front_ARB_stiff)/(C.front_width/2)))+(C.weight_dist)*(C.mass/2);
